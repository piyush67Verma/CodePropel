...
const ProblemPage = () => {
  const [problem, setProblem] = useState(null); // contains response.data
  const [selectedLanguage, setSelectedLanguage] = useState('javascript'); //contains selected language
  const [code, setCode] = useState(''); // initially conatins initialCode of selected language 
  ...
  let { problemId } = useParams(); //getting problemId from url/route 
  ...
  useEffect(() => {
    const fetchProblem = async () => {
      ...
      try {
        ...
        const initialCode = matched.initialCode || "";
        ...
        <div className="flex-1 flex flex-col">
          {activeRightTab === 'code' && (
            <div className="flex-1 flex flex-col">
              ...
              <div className="p-4 border-t border-base-300 flex justify-between">
                ...
                <div className="flex gap-2">
                  <button
                    className={`btn btn-outline btn-sm ${loading && runBtn ? 'loading' : ''}`}
                    onClick={handleRun}
                    disabled={loading}
                  >
                    Run
                  ...
                  <button
                    className={`btn btn-primary btn-sm ${loading && !runBtn? 'loading' : ''}`}
          ...
          {activeRightTab === 'testcase' && (
            <div className="flex-1 p-4 overflow-y-auto">
              ...
              {runResult ? (
                <div className={`alert ${runResult.success ? 'bg-green-500' : 'alert-error'} mb-4`}>
                  <div>
                    {runResult.success ? (
                      <div>
          ...
          {activeRightTab === 'result' && (
            <div className="flex-1 p-4 overflow-y-auto">
              ...
              {submitResult ? (
                <div className={`alert ${submitResult.accepted ? 'bg-green-500' : 'alert-error'}`}>
                  <div>
